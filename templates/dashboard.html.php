<div class="container">
    <h1>Mon  tableau de bord</h1>
    <div class="button-container">
        <a href="/?page=connexion"><button class="button">Se Déconnecter</button></a>
    </div>
    <nav class="navbar">
        <ul>
            <li><a href="/?page=liste-formation"><button class="button" title="Postuler à une formation">Postuler à une formation</button></a></li>
            <li><a href="/?page=absecnce-user"><button class="button" title="Mes absences">Mes absences</button></a></li>
            <li><a href="/?page=retard-user"><button class="button" title="Mes retards">Mes retards</button></a></li>
        </ul>
    </nav>
</div>